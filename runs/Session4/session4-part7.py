...
def main() -> int:
    ...
    print("\nFire next activation (should match fact 2):")
    env.run(limit=1)
    ...
    print("\nFire next activation (should match fact 3):")
    env.run(limit=1)
    print_agenda(env, "Agenda after firing third rule:")

    # 7. Allow duplication
    print("Allow duplication of facts...")
    ...
    print("\nFire activation from duplicated fact:")
    env.run(limit=1)
